...
RrFfMacScheduler::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
  ...
  do
  {
    ...
    {
      /////////////////////////////////////////////////////////////////////
      // cis549 Project 5   : Problem 1
      //
      // To apply a certain lte_DL_mcs_value then push_back this value using this command line:
      // "newDci.m_mcs.push_back( lte_dl_mcs_value )"
      // in the the original code below (if-statement below)
      //
      // If no mcs value is provided then use the  original code below between "EDIT START" and "EDIT END"
      //
      // Following three items would be useful for the work. 
      // How to find current time
      //     int current_time = ((params.m_sfnSf >> 4)* 10)+((0xF & params.m_sfnSf)-1);
      //
      // How to get ue_index
      //     int UE_id = newDci.m_rnti;
      //
      // How to build a MCS filename
      //     std::string filename = "LTE_DL_MCS_UE" + std::to_string(UE_id) + ".txt";    // using the UE_id parameter above
      // Or
      //     std::string filename = "LTE_DL_MCS_UE" + std::to_string(newDci.m_rnti) + ".txt";   // without using the UE_id parameter above
      //    
      //////////////////////////////////////////////////////////////////////
      ...
      else
      {
        ...
        std::ifstream mcsfile;
        int UE_id=newDci.m_rnti;
        std::string filename =  "scratch/LTE_DL_MCS_UE" + std::to_string(UE_id) + ".txt";
        int current_time = ((params.m_sfnSf >> 4)* 10)+((0xF & params.m_sfnSf)-1);
        

        mcsfile.open(filename);
        if (!mcsfile)
        {
        ...
        else
        {
                ...
                {
                        count=count+std::stoi(line.substr(2,3));
                        if (current_time<count)
                        {
                                newDci.m_mcs.push_back (std::stoi(line.substr(0,2)));
                                break;
                        }
...
RrFfMacScheduler::DoSchedUlTriggerReq (const struct FfMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
  ...
  do
  {
    ...
    else
    {
      ...
      uldci.m_mcs = m_amc->GetMcsFromCqi (cqi);   // uldci.m_mcs = lte_ul_mcs_value );
